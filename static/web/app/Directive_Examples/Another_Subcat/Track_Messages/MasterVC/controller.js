(function() {
    ...
    function FUSE_APP_NAMEController($websocket, $mdToast, $location, $timeout) {
            ...
            vm.ws.$on('$message', function(message) { // it listents for 'incoming event'
                if (message !== '--heartbeat--') {
                    ...
                    if (message['data']) {
                        ...
                        if (message['data'] === []) {
                            var audio2 = new Audio('/static/audio/beep_surrender.wav');
                            $mdToast.show(
                                $mdToast.simple()
                            ...
                            var audio = new Audio('/static/audio/beep_new_line_data.wav');
                            audio.play();
                        }
                    }
        ...
        function onDestroy() {
            var audio = new Audio('/static/audio/click_side_arm2.wav');
            audio.play();
            vm.closeWsCon();
        }
