...
def listWorkingDirectory(path):
    ...
    for (dirpath, dirnames, filenames) in walk(path):
        ...
        if len(dirnames) > 0:
            ...
            try:
                print(dirpath + dirnames[0] + filenames[0])
                print(open(dirpath + filenames[0],'r').read())
            except:
                print(' FAIL! ')
...
listWorkingDirectory('/usr/src/app/static/totala_files/')
...
unitListPrettyJSON.output_final.write(remove_comments(unitListPrettyJSON.outputConsole_unit))
