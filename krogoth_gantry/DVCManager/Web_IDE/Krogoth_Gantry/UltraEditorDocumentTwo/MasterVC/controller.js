...
(function() {
    ...
    function FUSE_APP_NAMEController($log, $scope, $http, $mdToast, $cookies, $state, $mdMenu,
        ...
        function getMasterViewCtrlDetail() {
            /*
            AKClassEditorComponentClone01.loadMasterInitializer(vm.selectedMaster)
        ...
        function getTemplatesHTML() {
            AKClassEditorComponentClone01.loadHTMLIncludeList(vm.objectList.name)
        ...
        function parallelRESTfulStart() {
            var cpuTask1 = GatherURIsAsyncClone01.async(vm.servicesPendingRequest, "Service")()
                .then(function(list) {
                    Array.prototype.push.apply(vm.pendingRESTfulRequests, list);
                });
            var cpuTask2 = GatherURIsAsyncClone01.async(vm.directivesPendingRequest, "Directive")()
                .then(function(list) {
                    Array.prototype.push.apply(vm.pendingRESTfulRequests, list);
                });
            var cpuTask3 = GatherURIsAsyncClone01.async(vm.slavesPendingRequest, "SlaveViewController")()
                .then(function(list) {
                    Array.prototype.push.apply(vm.pendingRESTfulRequests, list);
                });
            $q.all([cpuTask1,
        ...
        function parallelRESTfulReady() {
            ...
            for (var i = 0; i < vm.pendingRESTfulRequests.length; i++) {
                ...
                var cpuTaskX;
                cpuTaskX = BatchRequestsAsyncClone01.async(request_in.id, request_in.class, vm.treeData)()
                    .then(function(list) {
                        vm.treeData = list[0];
        ...
        function buildBreadCrumbs() {
            BreadCrumbsIDEClone01.cookBread($state.params.categoryId, $state.params.subCategoryId, $state.params.masterId)
        ...
        function scanAllVms() {
            ...
            syntaxAnalyzePropertiesVMClone01.getAllVms(vm.editorModel)
        ...
        function highlightSyntaxGetHtmlProperties() {
            codeHighlightIDEClone01.colorNgClick1(vm.editorModel)
        ...
        function highlightCollectedVMs() {
            syntaxAnalyzePropertiesVMClone01.highlightCurrentDocument(vm.editorModel);
        }
        /* △ △ △ RELOCATE ME TO A SEPARATE SERVICE △ △ △ */
        ...
        function renameObjectSubmit() {
            ...
            if (objectToRename === "Service") {
                fileNameChangerClone01.renameService(_0,
                        _1,
                        _2,
                ...
                DirectiveCRUDClone01.renameDirective(_0,
                        _1,
                        _2,
                        vm.treeData[vm.loadedParentIndex].nodes[vm.loadedIndex].name,
                        vm.renameObjectForm.new)
                ...
                TemplateCRUDClone01.renameTemplate(_0,
                        _1,
                        _2,
                        vm.treeData[vm.loadedParentIndex].nodes[vm.loadedIndex].name,
                        vm.renameObjectForm.new)
        ...
        function createNewComponentClick(ev, treeRoot) {
            ...
            ///var djangoModelName = SaveToSQLClone01.getRESTfulModelName(treeRoot.class);
            ...
            $mdDialog.show(confirm).then(function(new_name) {
                ...
                if (djangoModelName === "Directive") {
                    DirectiveCRUDClone01.createDirective(postPayload)
                    ...
                    fileNameChangerClone01.createService(postPayload)
                        .then(function(newTreeNode) {
                            $log.log("CREATE FINISHED");
                            $log.debug(newTreeNode);
                            const parentI = newTreeNode.parentIndex;
                    ...
                    TemplateCRUDClone01.createTemplate(postPayload)
                        .then(function(newTreeNode) {
                            $log.log("CREATE FINISHED");
                            $log.debug(newTreeNode);
                            const parentI = newTreeNode.parentIndex;
        ...
        function addNewComponentToMaster(siblings, newNode, djangoModelName, masterId) {
            SaveToSQLClone01.addSiblingToMaster(siblings, newNode, djangoModelName, masterId)
