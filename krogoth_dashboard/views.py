...
class getStorage(APIView):  # cat swaps
    def get(self, request, format=None):
        ...
        p = Popen(['df', '-h'], stdin=PIPE, stdout=PIPE, stderr=PIPE)
        output, err = p.communicate(b"input data that is passed to subprocess' stdin")
        rc = p.returncode
        ...
        #         print('LOOPING THROUGH STRING ARRAY: ')
        #         print(str1)
        ...
        storageDataColumns = final[0].replace('  ', ' ').replace(' ', '|').split('|')
        storageData = final[1].replace('  ', ' ').replace(' ', '|').split('|')
        ...
        for item in storageData:
            if item != '' and item != '/':
                arrayStorageData.append(item)

        for item in storageDataColumns:
            if item != '' and item != '/':
                arrayStorageColumns.append(item.replace("b'", ""))

        returnDict = {}
        returnDict['columns'] = arrayStorageColumns
        returnDict['data'] = arrayStorageData
